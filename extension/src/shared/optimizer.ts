...
export function optimizeShoppingPlan(
  ...
  const ingredientIds = [...new Set(inStockMatches.map((m) => m.ingredient_id))];
...
function optimizeSingleStore(
  ...
  for (const match of matches) {
    ...
    if (!storeScores.has(storeId)) {
      storeScores.set(storeId, { store: match.store, covered: new Set(), totalCost: 0, items: [] });
    }
    const entry = storeScores.get(storeId)!;
    const existing = entry.items.find((i) => i.ingredient_id === match.ingredient_id);
    if (!existing || match.match_score > existing.match_score) {
      if (existing) {
        entry.totalCost -= existing.price;
        entry.items = entry.items.filter((i) => i.ingredient_id !== match.ingredient_id);
      }
  ...
  return [{
    ...
    subtotal: Math.round(bestStore.items.reduce((sum, m) => sum + m.price, 0) * 100) / 100,
  }];
}
...
function optimizeCheapest(
  ...
  for (const ingId of ingredientIds) {
    const candidates = matches.filter((m) => m.ingredient_id === ingId).sort((a, b) => a.price - b.price);
    if (candidates.length > 0) assignment.set(ingId, candidates[0]);
  }
...
function optimizeFewestStops(
  ...
  while (uncovered.size > 0 && selectedStores.length < maxStores) {
    ...
    for (const storeId of storeIds) {
      ...
      const covers = matches.filter((m) => m.store.id === storeId && uncovered.has(m.ingredient_id)).length;
      if (covers > bestCoverage) { bestCoverage = covers; bestStoreId = storeId; }
    }
    ...
    for (const match of storeMatches) {
      if (uncovered.has(match.ingredient_id)) {
        ...
        if (!existing || match.match_score > existing.match_score) assignment.set(match.ingredient_id, match);
...
function optimizeBalanced(
  ...
  const assignment = new Map<string, ProductMatch>();
  for (const ingId of ingredientIds) {
    const candidates = matches.filter((m) => m.ingredient_id === ingId).sort((a, b) => {
  ...
  if (storeCount > maxStores) return optimizeFewestStops(matches, ingredientIds, maxStores);
  return buildStopsFromAssignment(assignment, maxStores);
}
...
function buildStopsFromAssignment(
  ...
  for (const match of assignment.values()) {
    const storeId = match.store.id;
    if (!storeGroups.has(storeId)) storeGroups.set(storeId, { store: match.store, items: [] });
    storeGroups.get(storeId)!.items.push(match);
  }

  const sorted = [...storeGroups.values()].sort((a, b) => a.store.distance_miles - b.store.distance_miles);
  return sorted.map((group, i) => ({
    order: i + 1,
    store: group.store,
    items: group.items.map(toShoppingItem),
    subtotal: Math.round(group.items.reduce((sum, m) => sum + m.price, 0) * 100) / 100,
  }));
}
